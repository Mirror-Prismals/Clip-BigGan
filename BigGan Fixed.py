...
This version uses a loss formulation and regularization that help prevent an initially negative loss
and overly saturated ("deepfried") images.
...
def define_parameters():
    # For quicker feedback, we use a reduced number of optimization steps.
    params = {
        "prompt": "A golden retriever in a park",
        "num_steps": 50,            # Reduced number of optimization steps for quick feedback.
        "lr": 0.01,                 # Lower learning rate for smoother optimization.
        "truncation": 0.4,
...
def postprocess_biggan_output(img_tensor):
    # BigGAN returns images as tensors in [0, 1]. Convert to a PIL image.
...
def optimize_biggan(prompt, params, update_callback, log_callback, should_stop):
    ...
    for step in range(1, params["num_steps"] + 1):
        ...
        # Compute cosine similarity and define loss.
        cosine_sim = F.cosine_similarity(image_features, text_features).mean()
        loss = 1 - cosine_sim  # Loss is positive and decreases toward zero as similarity increases.
        ...
        # Clamp latent vector to keep it near the prior distribution.
        z.data.clamp_(-2, 2)
